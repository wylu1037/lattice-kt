...
class LatticeTest {
    ...
    @Test
    fun `deploy counter contract transaction`() {
        val bytecode =
    ...
    fun `execute contract increment counter`() {
        ...
        val tx = Transaction(
            ...
            timestamp = Instant.now().epochSecond,
            owner = Address(ACCOUNT_ADDRESS_STR),
            linker = Address("zltc_d1pTRCCH2F6McFCmXYCB743L7spuNtw31"),
        ...
        // val hash = httpApi.sendRawTBlock(tx)
        val receipt = httpApi.preExecuteContract(tx)
        // val receipt = httpApi.getReceipt(hash)
        println(gson.toJson(receipt))
    }
